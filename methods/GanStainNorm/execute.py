...
#         '--exp_root', help='Root path to experiments', type=str, default="output")
#     train_parser.add_argument(
#         '--data_root', help='Root path to data, usually on temporary disk', type=str, default="input")
#     train_parser.add_argument(
...
#         '--exp_path', help='Path to specific experiment', type=str, default="output")
#     continue_train_parser.add_argument(
#         '--data_root', help='Root path to data, usually on temporary disk', type=str, default="input")
#     continue_train_parser.add_argument(
...
def main():
    ...
    train_parser.add_argument(
        "--multi_gpu",
        help="Enable training with multiple GPUs",
        action="store_true",
    )
    add_config_path_to_subparser(train_parser)
    train_parser.set_defaults(func=train)

    continue_train_parser = subparsers.add_parser(
        "continue_train",
        help="Continue training",
    )
    continue_train_parser.add_argument(
        "--exp_path",
        help="Path to specific experiment",
        type=str,
        default="output",
    )
    continue_train_parser.add_argument(
    ...
    continue_train_parser.set_defaults(
        func=continue_train
    )
    ...
    merge_tiles_parser = subparsers.add_parser(
        "merge_tiles",
        help="Merge tiles after inference",
    )
    merge_tiles_parser.add_argument(
        "--exp_path",
        help="Path to specific experiment",
    ...
    merge_tiles_parser.add_argument(
        "--inference_root",
        help="Root path to inference generated data",
        type=str,
    )
    merge_tiles_parser.add_argument(
        "--multiprocess",
        help="Run WSI tile merging in parallel",
        action="store_true",
    )
    add_config_path_to_subparser(
        merge_tiles_parser
    )
    merge_tiles_parser.set_defaults(
        func=merge_tile_to_generate_wsi
    )

    tile_images_parser = subparsers.add_parser(
        "tile",
        help="Tile WSIs",
    )
    tile_images_parser.add_argument(
        "--multiprocess",
        help="Parallelize tiling",
        action="store_true",
    )
    add_config_path_to_subparser(
        tile_images_parser
    )
    tile_images_parser.set_defaults(
        func=tile_wsi
    )

    untar_tiles_parser = subparsers.add_parser(
        "untar_tiles",
        help="Untar tiles",
    )
    untar_tiles_parser.add_argument(
        "--src_path",
        help="Path to source tar files",
        type=str,
        required=True,
    )
    untar_tiles_parser.add_argument(
    ...
    untar_tiles_parser.set_defaults(
        func=untar
    )

    downsample_parser = subparsers.add_parser(
        "downsample",
        help="Run TIFF downsampling",
    )
    downsample_parser.add_argument(
        "--exp_path",
        help="Path to specific experiment",
    ...
    downsample_parser.set_defaults(
        func=downsample_wsi
    )

    # Default PyCharm run: inference
    if len(sys.argv) == 1:
        sys.argv.extend(
            [
                "inference",
                "--config_path",
                "config/inference/"
                "config_inference_cyclegan_resnet_a.json",
                "--exp_path",
                "output/CycleGan/a",
                "--data_root",
                "input",
